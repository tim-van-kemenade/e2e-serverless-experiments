...
function_times = []
curl_times = []
with open("data/overhead.txt") as overhead_file:
    lines = overhead_file.readlines()
    for i in range(0, len(lines) - 2, 3):
        f_time = float(lines[i + 1]) / 1e9 - float(lines[i]) / 1e9
        function_times.append(f_time)
        curl_times.append(float(lines[i + 2]))
...
pod_times = []
with open("data/pod_log.txt") as pod_log_file:
...
with open("csv/data_line.csv", "w") as line_file:
...
with open("csv/data_bar.csv", "w") as line_file:
...
with open("csv/data_boxplot.csv", "w") as boxplot_file:
    ...
    for i in range(100):
        ...
        # execution_pod_time.append(str(float(pod_times[i])))
